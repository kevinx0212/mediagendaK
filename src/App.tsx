          ...
          <div className="max-w-7xl mx-auto">
            ...
            {currentView === 'atc' && (profile.role === 'admin') && (
              <ATCView
                ...
                users={users}
                isDarkMode={isDarkMode}
                onReply={(patientName: string, patientDni: string, text: string) => sendChatMessage(patientName, patientDni, text, 'admin')}
                onMarkRead={markRead}
              />
            )}
            {currentView === 'mensajes' && profile.role === 'patient' && (
              <MensajesView
                ...
                isDarkMode={isDarkMode}
                onSend={(text: string) => sendChatMessage(profile.name, profile.dni || '', text, 'patient')}
              />
            )}
            {currentView === 'ajustes' && (
